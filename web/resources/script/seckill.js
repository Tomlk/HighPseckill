        ...
        $.post(seckill.URL.exposer(seckillId),{},function(result){
            ...
            if(result && result['success']){
                ...
                if(exposer['exposed']){
                    ...
                    var end=exposer['end'];
                    //重新进入计时逻辑
                    seckill.countdown(seckillId,now,start,end);//每个机器运行时间有偏差，时间纠正
    ...
    countdown:function(seckillId,nowTime,startTime,endTime){
        ...
        if(nowTime>endTime){
            ...
            seckillBox.countdown(killTime,function (event){
                //时间格式
                var fortmat=event.strftime('秒杀倒计时: %D天 %H时 %M分 %S秒');
                seckillBox.html(fortmat);
